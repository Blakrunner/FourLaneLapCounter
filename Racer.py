...
class Racer(QObject):
    ...
    def checkRacePosition(self):
        ...
        if return_bool is True:
               maxlap = max(laps)
               nextracelap = None
               if len(self.race_position) > 1:
                   nextracelap = self.race_position[j-1][1]
               if maxlap < self.lap:
                   self.race_position.pop(j)
                   self.race_position.insert(0,[self.name,self.lap])
               elif not nextracelap is None and nextracelap < self.lap:
                   self.race_position.pop(j)
                   self.race_position.insert(j - 1,[self.name,self.lap])
               else:
                   self.race_position.pop(j)
                   self.race_position.insert(j,[self.name,self.lap])
