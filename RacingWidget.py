...
from LapTimingDataTables import ChampionshipLapTimes, RaceLapTimes
from LeaderboardTable import Leaderboard
...
class RacingWidget(QDialog):
    def __init__(self, parent=None):
        ...
        exit_button.setStyleSheet('height: 100px; border: 1px solid; border-radius: 10px;')
        exit_button.setText('Main Menu')
        exit_button.clicked.connect(self.onExit)
        options.addWidget(exit_button)
        
        free_button = QPushButton()
        free_button.setStyleSheet('height: 100px;')
    ...
    def onFreeRace(self):
        ...
        self.child_camera.setDescription("Keep count of your laps.")
    ...
    def onRace(self):
        ...
        if self.parent is None:
            self.errorNotMain()
            return
        if self.child_camera.video_widget.numberRacers() == 3:
