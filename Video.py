    ...
    def saveLBdata(self):
        ...
        try:
            ...
            with open(fllc.DATABASE_PATH+filename, 'w', newline='') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=fields)
                writer.writeheader()
                writer.writerow({"name":fllc.RACER1NAME,"lane":1,"bestLap":self.laneonetimes[0] - min(self.laneonetimes),"points":self.getPoints(fllc.RACER1NAME)})
                writer.writerow({"name":fllc.RACER2NAME,"lane":2,"bestLap":self.lanetwotimes[0] - min(self.lanetwotimes),"points":self.getPoints(fllc.RACER2NAME)})
                writer.writerow({"name":fllc.RACER3NAME,"lane":3,"bestLap":self.lanethreetimes[0] - min(self.lanethreetimes),"points":self.getPoints(fllc.RACER3NAME)})
                writer.writerow({"name":fllc.RACER4NAME,"lane":4,"bestLap":self.lanefourtimes[0] - min(self.lanefourtimes),"points":self.getPoints(fllc.RACER4NAME)})
            csvfile.close()
...
class VideoWidget(QWidget):
        ...
        if self.show_time:
            ...
            if len(fllc.RACER1) > 0:
                ...
                if len(fllc.RACER1) > 1:
                    self.checkTrackRecord(min(fllc.RACER1))
            ...
            if len(fllc.RACER2) > 0:
                ...
                if len(fllc.RACER2) > 1:
                    self.checkTrackRecord(min(fllc.RACER2))
                    painter.setPen(QColor(Qt.blue))
            ...
            if len(fllc.RACER3) > 0:
                ...
                if len(fllc.RACER3) > 1:
                    self.checkTrackRecord(min(fllc.RACER3))
                    painter.setPen(QColor(Qt.blue))
            ...
            if len(fllc.RACER4) > 0:
                ...
                if len(fllc.RACER4) > 1:
                    self.checkTrackRecord(min(fllc.RACER4))
                    painter.setPen(QColor(Qt.blue))
    ...
    def checkTrackRecord(self, value):
