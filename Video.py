...
class RecordVideo(QObject):
        ...
        if read:
            ...
            for cur in contours:
                if cv2.contourArea(cur) < fllc.CONTOUR_AREA:
                    continue
                (x, _, w, _) = cv2.boundingRect(cur)
                if w < fllc.ANTI_SHAKE_MIN or w > fllc.ANTI_SHAKE_MAX:
                    continue
               # fllc.MESSAGE = " w="+str(w) + fllc.MESSAGE
                #fllc.SHAPES.append(Rectangle(w, h, QPoint(x, y), QColor(128, 255, 128), 1))
                #fllc.var_motion += 1
    ...
    def clearLapData(self):
        self.laneonetimes.clear()
        self.lanetwotimes.clear()
        self.lanethreetimes.clear()
        self.lanefourtimes.clear()
    ...
    def saveLapData(self):
        ...
        if self.lap_data_loaded is False:
            self.setData()
        self.reverseLapData()
    ...
    def saveLBdata(self):
        ...
        try:
            ...
            with open(fllc.DATABASE_PATH+filename, 'w', newline='') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=fields)
                writer.writeheader()
                writer.writerow({"name":fllc.RACER1NAME,"lane":1,"bestLap":self.getLaneValue(1,self.laneonetimes),"points":self.getPoints(fllc.RACER1NAME)})
                writer.writerow({"name":fllc.RACER2NAME,"lane":2,"bestLap":self.getLaneValue(2,self.lanetwotimes),"points":self.getPoints(fllc.RACER2NAME)})
                writer.writerow({"name":fllc.RACER3NAME,"lane":3,"bestLap":self.getLaneValue(3,self.lanethreetimes),"points":self.getPoints(fllc.RACER3NAME)})
                writer.writerow({"name":fllc.RACER4NAME,"lane":4,"bestLap":self.getLaneValue(4,self.lanefourtimes),"points":self.getPoints(fllc.RACER4NAME)})
            csvfile.close()
...
class VideoWidget(QWidget):
    ...
    def setRacers(self):
        number_racers = self.numberRacers()
        self.racer1.setName(fllc.RACER1NAME)
        self.racer1.setMaxLaps(fllc.MAX_LAPS)
        self.racer1.setCarColor(fllc.RACER1CARCOLOR)
        self.racer1.setNumberRacers(number_racers)
        self.racer2.setName(fllc.RACER2NAME)
        self.racer2.setMaxLaps(fllc.MAX_LAPS)
        self.racer2.setCarColor(fllc.RACER2CARCOLOR)
        self.racer2.setNumberRacers(number_racers)
        self.racer3.setName(fllc.RACER3NAME)
        self.racer3.setMaxLaps(fllc.MAX_LAPS)
        self.racer3.setCarColor(fllc.RACER3CARCOLOR)
        self.racer3.setNumberRacers(number_racers)
        self.racer4.setName(fllc.RACER4NAME)
        self.racer4.setMaxLaps(fllc.MAX_LAPS)
        self.racer4.setCarColor(fllc.RACER4CARCOLOR)
        self.racer4.setNumberRacers(number_racers)
    ...
    def paintEvent(self, event):
        if self.racers_set is False:
            self.setRacers()
            self.racers_set = True
        ...
        number_racers = self.numberRacers()
        if fllc.var_motion >= number_racers:
            fllc.var_motion = 0
            fllc.SHAPES.clear()
        '''
        ...
        if fllc.start_loop != 0:
            ...
            painter.drawText(10,1770,"cycle:"+str(taken)+"ms")
    ...
    def drawOverlay(self, qpainter, w, h):
        ...
        lanewidth = w/4
        # draw frames
        painter.setPen(QPen(QColor(Qt.white),5))
        painter.drawRect(0,0,lanewidth,h)
        painter.drawRect(lanewidth,0,lanewidth,h)
        painter.drawRect(lanewidth*2,0,lanewidth,h)
        painter.drawRect(lanewidth*3,0,lanewidth,h)
        # draw trigger line for visual only
        painter.setPen(QPen(QColor(fllc.RACER1COLOR),5))
        painter.drawRect(offset,100,lanewidth-20,5)
        painter.setPen(QPen(QColor(fllc.RACER2COLOR),5))
        painter.drawRect(lanewidth+offset,100,lanewidth-20,5)
        painter.setPen(QPen(QColor(fllc.RACER3COLOR),5))
        painter.drawRect((lanewidth*2)+offset,100,lanewidth-20,5)
        painter.setPen(QPen(QColor(fllc.RACER4COLOR),5))
        painter.drawRect((lanewidth*3)+offset,100,lanewidth-20,5)
        
        painter.setFont(QFont('Roboto',20))
        if self.show_names:
            if self.racer1.getName() != "":
                painter.setPen(self.racer1.getColor())
                painter.drawText(20,70,self.racer1.getName())
            if self.racer2.getName() != "":
                painter.setPen(self.racer2.getColor())
                painter.drawText(lanewidth+20,70,self.racer2.getName())
            if self.racer3.getName() != "":
                painter.setPen(self.racer3.getColor())
                painter.drawText((lanewidth*2)+20,70,self.racer3.getName())
            if self.racer4.getName() != "":
                painter.setPen(self.racer4.getColor())
                painter.drawText((lanewidth*3)+20,70,self.racer4.getName())
        if self.show_laps:
        ...
        # Race Positions center leaderboard
        if self.show_race_positions:
            painter.fillRect(lanewidth,550,w-(lanewidth*2),500,Qt.black)
            pen = QPen(Qt.white,7)
            painter.setPen(pen)
            painter.drawRect(lanewidth,550,w-(lanewidth*2),500)
            painter.drawRect(lanewidth,550,w-(lanewidth*2),150)
            painter.setFont(QFont('Roboto',20))
            painter.setPen(QColor(Qt.white))
            painter.drawText(int(lanewidth*1.17),650,"RACE POSITION")
            _cnt = 0
            if len(self.racer4.race_position) > 0:
                for name, _ in self.racer4.race_position:
                    painter.drawText(int(lanewidth*1.17),790+(70 * _cnt),str(_cnt + 1)+".  "+name)
                    _cnt += 1
