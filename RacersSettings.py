...
class Setup(QDialog):
    ...
    def SaveSetup(self):
        ...
        self.SaveCSV()
        fllc.saveSettings()
        self.parent.child_racing.child_camera.video_widget.setRacers()
        self.parent.show()
        self.reject()
    
    def OnIndexChanged(self):
        next_preset = self.preset_box.currentData()
        if next_preset == "" or next_preset is None:
            return
        if isfile(join(fllc.PRESET_PATH,next_preset)):
            return
